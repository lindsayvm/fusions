...
fn = list.files(path = "/DATA/share/Voesties/data/HMF/update_5/linx/",
                ...
                full.names = TRUE,  
                recursive = TRUE)
fn = fn[gsub(pattern = ".*\\/|.linx.fusion.tsv", "", fn) %in% id]
...
colnames(df) = c("PatientID",colnames(ls[[1]]))
#Info: number of fusions per patient
info.df =  as.data.frame(matrix(ncol = 5, nrow = 1))
colnames(info.df) = c("PatientID","n_fusion_wgs","n_longJunction","n_outofframe","n_exonintron") 
#Retrieve fusion gene info for each patients
for(i in 1:length(ls)){
  print(i)
  #open each sampleId (patient) file  
  tmp.df = ls[[i]]
  tmp.df$PatientID = rep(names(ls[i]), nrow(tmp.df))
  #make one large df with all fusion gene data
  df = rbind(df, tmp.df)
  #number of fusions within the patient file
  all.df = tmp.df
  # number of long junctions fusions 
  tmp.df$junctionlength = abs(tmp.df$ThreePrimeBreakendId - tmp.df$FivePrimeBreakendId)
  longJunction.df = tmp.df[tmp.df$junctionlength > 100, ]
  #in-frame / out-of-frame
  outofframe.df = tmp.df[tmp.df$Phased == "OUT_OF_FRAME", ]
  #exon fused to intron
  exonintron.df = tmp.df[gsub("[1-9].*$","", tmp.df$GeneContextStart) %in% c("Exon ", "Promoter Region") &
                         !gsub("[1-9].*$","", tmp.df$GeneContextEnd) == "Exon "  , ]
  newRow = c(names(ls)[i], length(unique(all.df$Name)), 
             nrow(longJunction.df), 
             nrow(outofframe.df), 
             nrow(exonintron.df))
  info.df = rbind(newRow,info.df)  
}
info.df = info.df[!is.na(info.df$PatientID), ]
info.df[-1] = sapply(info.df[-1],as.integer)
str(info.df)

#write.table(info.df, file = "dr043/update5/fusion_wgs_info_extended_lung.tsv", quote=FALSE, sep='\t',row.names = FALSE, col.names = TRUE)
...
reg = lm(n_exonintron ~ n_fusion_wgs, data = info.df)
pp = ggplot(info.df, aes(x = n_fusion_wgs, y = n_exonintron, text = PatientID)) + 
...
length(df$Name) #3864 total
length(df$Name) - length(unique(df$Name)) #298 double
( (length(df$Name) - length(unique(df$Name))) /(length(unique(df$Name))) ) * 100 #8 of the fusion genes is prevalent > 2

# Frequency table per fusion
freq.df = plyr::count(df, "Name")
freq.df = freq.df[order(freq.df$freq, decreasing = TRUE), ] #ordered
# Most fusions are occur only once, most under 25x among all patients, highest to 800x among all patients
...
plyr::count(gsub(".[0-9]$| ","",df$GeneContextStart))
plyr::count(gsub(".[0-9]$| ","",df$GeneContextEnd))
phased.df$prop = round(phased.df$freq/sum(phased.df$freq),2)
phased.df
...
p3 = ggplot(freq.df, aes(x= freq, y= junctionlength_mean)) + 
  geom_point(shape=20, size =0.5)
ggplotly(p3)


#make a log scale
freq.df$log_junctionlength_mean = log(freq.df$junctionlength_mean)
...
#plot scatter
p4 = ggplot(freq.df, aes(x= freq, y= log_junctionlength_mean, text = Name)) + 
  geom_point(shape=1, size = 0.7, fill = "#69b3a2", color = "#69b3a2", alpha = 0.7 )+
  theme_ipsum()
...
p5 = ggplot(internalfusion.df, aes(x= freq, y= log_junctionlength_mean, text = Name)) + 
  geom_point(shape=1, size = 0.7, fill = "#69b3a2", color = "#69b3a2", alpha = 0.7 ) +
  theme_ipsum() +
  xlim(0,20) +
  ylim(0,10)
...
p6 = ggplot(externalfusion.df, aes(x= freq, y= log_junctionlength_mean, text = Name)) + 
  geom_point(shape=1, size = 0.7, fill = "#69b3a2", color = "#69b3a2", alpha = 0.7 ) +
  theme_ipsum() +
  xlim(0,20) +
  ylim(0,10)
